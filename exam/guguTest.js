...
app.get("/gugu", (req, res) => {
  ...
  list += `            margin: auto;`;
  list += `        }`;
  list += `        div { margin:0 auto }`;
  list += `        select, button { display: inline-block;} `;
  list += `        th, tr, td {`;
  ...
  list += `    <option value="100">전체</option>`;
  for (let i = 2; i < 10; i++) {
    list += `        <option value="${i}">${i}단</option>`;
  }
  list += `    </select>`;
  list += `    <button onclick="location.href='/'">초기 페이지로</button></div><br><br>`;
  list += `    <table>`;
  list += `        <tr>`;
