    ...
    db.query(query, [no], (err, results) => {
      ...
      const bcontent = `
        <div id="firstLine">
          ...
          <div id="firstLineR">
            <div id="wdate">작성일시 <input type="text" id="wdatei" name="idate" class="inpel" value="${todo.created_at}" readonly/></div>
            <div id="whit">조회수 <input type="text" id="whiti" name="ihit" class="inpel" value="${todo.view_count}" readonly/></div>
          </div>             
        </div>
        <div id="secondLine">제목 <input type="text" id="wtitlei" name="ititle" class="inpel" value="${todo.title}" readonly/></div>
        <div id="thirthLine">작성자 <input type="text" id="wwriteri" name="iwriter" class="inpel" value="${todo.nick}" readonly/></div>
        <div id="forthLine">
          <h5>내용</h5>
          <textarea id="wcontenti" name="iwcontent" readonly>${todo.content}</textarea>
        </div>`;
      res.send(bcontent);
    });
  });
